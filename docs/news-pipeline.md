...
| Route | Auth | Behavior |
| --- | --- | --- |
| `GET /api/cron/fetch-news` | `CRON_SECRET` via the shared cron gate; fails closed | Lock → fetch all → store → prune Redis. Returns `{success, fetched, stored, duplicates, pruned}`. Scheduled daily by `vercel.json`. |
| `POST /api/news/fetch` | session + `news` section | The `/news` "Fetch Now" button. Same run as the cron (`src/lib/news-fetch.ts`), same Redis mutex — a concurrent call gets `{message: "Fetch already in progress"}`. |
| `GET /api/topics` | session + `news` section | Clusters articles from the last `days` (default 3, clamped 1–30). Redis cache `cache:topics` (1h TTL), invalidated when `fetch:last` or clustering mode changes. `?refresh=1` forces a re-cluster and is **admin-only** (it's a paid LLM call in `ai`/`hybrid` mode). Returns `{topics, totalArticles, lastFetch, clusteringMode, cached}`. |
| `GET /api/stats` | session + `news` section | 48h hourly article counts. |
...
- `GET /api/plausible` checks the session and the `reports` section per call, using `getSession({fromClaimsOnly: true})` to skip the Logto `/userinfo` round-trip on this hot path (10+ calls per render). It used to trust the middleware cookie gate alone, which any forged cookie satisfies.

## `/analyze`

Natural-language Q&A over Plausible via Claude tool use.

- `POST /api/analyze` (SSE stream, `maxDuration=120`, requires the `reports` section): agent loop, model `claude-sonnet-4-6`, max 8 iterations, 20-turn history window. Emits `step` events (`ai_request`, `tool_request`, `tool_response`, `answer`, …) rendered by `AnalyzeView`/`Transcript`/`StepLog`.
...
- A cache miss re-clusters regardless of who asked — `?refresh=1` being admin-only caps *forced* re-clusters, not the once-per-fetch-cycle one.
