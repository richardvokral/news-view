...
- `insights/` — long-horizon article stats: chunked weekly Plausible backfill (`backfill.ts`), URL/section parsing (`paths.ts`), ISO-week maths (`weeks.ts`), Postgres access (`store.ts`), and the AI theme analysis (`analyze.ts` + `providers.ts` + `ground.ts`, which recomputes every number the model might otherwise invent).
  - Headline work sits alongside it: `titleAnalysis.ts` (cohort SQL), `titleRun.ts` (analysis + rewriter orchestration), `titlePrompts.ts` / `titleProviders.ts` (Czech contracts and schemas), `titleFetch.ts` (og:title enrichment). Saved playbooks live in `insights_prompts` with `kind='title_rewrite'`; persisted headline-form tags live in `insights_title_tags` (`titleTags.ts`) and are what make two analyses comparable — the model's own `patterns` are re-derived per run and their labels drift.
