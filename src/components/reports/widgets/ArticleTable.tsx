...
import type { ArticleBreakdownWidgetConfig } from "@/types/dashboard";
...
function parseArticleName(page: string): string {
  const slug = page.replace(/^\/clanek\//, "");
...
interface Props {
  config: ArticleBreakdownWidgetConfig;
  dateRange: DateRangeValue;
  siteId: string;
}

export default function ArticleTable({ config, dateRange, siteId }: Props) {
    ...
    fetch(`/api/plausible?${params}`)
      ...
      .then((json) => {
        ...
        const articles = results
          .filter((r) => String(r.page).startsWith("/clanek/"))
      ...
      .then((breakdownData) => {
        ...
        return Promise.all(
          uniqueValues.map(async (val) => {
            ...
            return {
              value: Number(val),
              pages: (json.results || []) as Record<string, string | number>[],
            };
          })
        ).then((seriesResults) => {
          const byPage: Record<string, { total: number; events: number }> = {};
          for (const { value, pages } of seriesResults) {
            for (const row of pages) {
              const page = String(row.page);
              if (!page.startsWith("/clanek/")) continue;
              if (!byPage[page]) byPage[page] = { total: 0, events: 0 };
              const count = Number(row[config.metric]) || 0;
              byPage[page].total += value * count;
              byPage[page].events += count;
            }
          }
          const articles: ArticleRow[] = Object.entries(byPage)
            .map(([page, vals]) => ({
              page,
              name: parseArticleName(page),
              url: `${config.baseUrl}${page}`,
